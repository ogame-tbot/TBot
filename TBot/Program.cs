						...
						if (idealShips <= origin.Ships.GetAmount(preferredShip)) {
							...
							if (destination.Coordinate.Type == Celestials.Planet) {
								...
								if (
									...
									Helpers.WriteLog(LogType.Info, LogSender.Brain, "Sending " + ships.ToString() + " with " + missingResources.TotalResources.ToString() + " from " + origin.ToString() + " to " + destination.ToString());
